...
export default function Payments() {
  ...
  const fetchData = async () => {
    ...
    const { data: bundleData } = await supabase
      .from('bundle_offer')
      .select('bundle_id, total_price, service_date, status')
      .eq('customer_id', user.id)
      .in('status', ['accepted', 'completed'])
    ...
    const paidBundleIds = (paymentData || [])
      .filter(p => p.payment_status === 'completed')
      .map(p => p.bundle_offer?.bundle_id)
      .filter(Boolean)
  ...
  // Get amount for selected item
  const getSelectedAmount = () => {
    if (!selectedItem) return 0
    if (selectedItem.startsWith('booking_')) {
      const id = selectedItem.replace('booking_', '')
      return unpaidBookings.find(b => b.booking_id === id)?.service?.price || 0
    }
    if (selectedItem.startsWith('bundle_')) {
      const id = selectedItem.replace('bundle_', '')
      return unpaidBundles.find(b => b.bundle_id === id)?.total_price || 0
    }
    return 0
  }

  const handlePayment = async () => {
    if (!selectedItem) { alert('Please select a booking or bundle to pay for'); return }
    setPaying(true)
    ...
    const insertData = {
      ...
      booking_id: isBundle ? null : actualId,
      bundle_id: isBundle ? actualId : null,
    }

    const { error } = await supabase.from('payment').insert(insertData)
        ...
        {totalUnpaid > 0 && (
          <div style={{ background: '#fff', border: '1px solid #d4d2cc', borderRadius: 10, padding: 20, marginBottom: 24, boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}>
            ...
            <div style={{ background: '#e8ecff', border: '1px solid #c5cfff', borderRadius: 8, padding: '10px 14px', fontSize: 12, color: '#2a3fd4', marginBottom: 16 }}>
              💳 You have {totalUnpaid} item(s) awaiting payment
              {unpaidBookings.length > 0 && ` · ${unpaidBookings.length} booking(s)`}
              {unpaidBundles.length > 0 && ` · ${unpaidBundles.length} bundle(s)`}
            </div>
            ...
            <div style={{ marginBottom: 16 }}>
              <label style={{ fontSize: 12, fontWeight: 500, color: '#6b6860', textTransform: 'uppercase', letterSpacing: 0.5, display: 'block', marginBottom: 6 }}>
                Select Booking or Bundle
              </label>
              <select value={selectedItem} onChange={e => setSelectedItem(e.target.value)}
                style={{ width: '100%', border: '1.5px dashed #d4d2cc', borderRadius: 7, padding: '10px 13px', fontSize: 13, background: '#f4f3f0', outline: 'none', maxWidth: 500 }}>
                <option value="">— Choose a booking or bundle —</option>

                {unpaidBookings.length > 0 && (
                  <optgroup label="📅 Bookings">
                    {unpaidBookings.map(b => (
                      <option key={b.booking_id} value={`booking_${b.booking_id}`}>
                ...
                {unpaidBundles.length > 0 && (
                  <optgroup label="📦 Bundles">
                    {unpaidBundles.map(b => (
                      <option key={b.bundle_id} value={`bundle_${b.bundle_id}`}>
                        Bundle — {b.service_date} — Rs. {Number(b.total_price || 0).toLocaleString()}
                      </option>
                    ))}
                  </optgroup>
                )}
            ...
            {selectedItem && (
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 15, fontWeight: 700, marginBottom: 16, padding: '12px 0', borderTop: '1px solid #d4d2cc' }}>
                ...
                <span style={{ color: '#3d5afe' }}>Rs. {Number(getSelectedAmount()).toLocaleString()}</span>
              </div>
            )}
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: '#f4f3f0' }}>
                  {['Transaction ID', 'For', 'Amount', 'Method', 'Date', 'Status'].map(h => (
              ...
              <tbody>
                {payments.map(p => {
                  ...
                  const forLabel = p.booking?.service?.service_name
                    ? `📅 ${p.booking.service.service_name}`
                    : p.bundle_offer?.bundle_id
                    ? `📦 Bundle`
                    : '—'
                  return (
